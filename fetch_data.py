...
import re
#The comment at the top of the program allows python to properly encode non-ASCII characters.
...
def cleaner(link, songname):
	#The link_finder through filth I basically just ripped off of the Beautiful Soup webpage.  So I'm afraid I dont know much on how it works.  The "encode('utf-8')" is more instructions for python on how to
	#encode the non-ASCII characters, 
	link_finder = urllib2.urlopen('%s' % (link))
	link_opener = link_finder.read()
	filth = BeautifulSoup(link_opener,'html.parser')
	semi_clean = filth.get_text().encode('utf-8')
...
"""
	...
	#group(0).  If it finds any character that isn't in the list, it will not add it to group(0).
	#It will continue adding and discarding characters until it reaches stoplyric.  So then group(0) holds all of the text lyrics we want without the code inside them.
